...
class TestStoreOperations:
    ...
    def test_shipping_management(self):
        ...
        # קבלת המזהה של אזור המשלוח שנוצר
        response = self.action_agent.wcapi.get("shipping/zones")
        assert response.status_code == 200, "נכשל בקבלת רשימת אזורי משלוח"
        zones = response.json()
        zone = next((z for z in zones if z["name"] == zone_data["name"]), None)
        assert zone is not None, "אזור המשלוח לא נמצא"
        zone_id = zone["id"]
    ...
    def tearDown(self):
        """ניקוי סביבת הבדיקה"""
        # קבלת כל המוצרים
        response = self.action_agent.wcapi.get("products", params={"per_page": 100})
        if response.status_code == 200:
            products = response.json()
            # מחיקת כל המוצרים שמתחילים ב-Test
            for product in products:
                if product["name"].startswith("Test"):
                    self.action_agent.wcapi.delete(f"products/{product['id']}", params={"force": True})
                    logger.info(f"נמחק מוצר בדיקה: {product['name']}")
