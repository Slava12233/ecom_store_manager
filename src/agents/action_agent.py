...
class ActionAgent:
    ...
    def _extract_product_info(self, message: str) -> Optional[Dict[str, Any]]:
        ...
        Examples: 
        - 'הוסף מוצר חדש בשם חולצה במחיר 70'
        - 'צור מוצר חדש נעליים אדומות מחיר 299 כמות 5'
        - 'יצירת מוצר חדש מכנסיים שחורים ב-150 שקל'
        """
        # ניסיון למצוא שם מוצר
        name_patterns = [
            r'בשם\s+([^\d]+?)\s+במחיר',  # הפורמט הישן
            r'(?:צור|הוסף|יצירת)\s+מוצר\s+חדש\s+([^\d]+?)(?:\s+מחיר|\s+ב-|\s+במחיר|\s+כמות)',  # פורמט חדש
            r'(?:צור|הוסף|יצירת)\s+מוצר\s+([^\d]+?)(?:\s+מחיר|\s+ב-|\s+במחיר|\s+כמות)'  # ללא המילה "חדש"
        ]
        ...
        # ניסיון למצוא מחיר
        price_patterns = [
            r'במחיר\s+(\d+)',  # הפורמט הישן
            r'מחיר\s+(\d+)',   # פורמט חדש
            r'ב-(\d+)\s+שקל'   # פורמט נוסף
        ]
        ...
        # ניסיון למצוא כמות
        quantity_match = re.search(r'כמות\s+(\d+)', message)
        ...
        if name and price:
            ...
            # הוספת כמות אם צוינה
            if quantity_match:
                product_data["stock_quantity"] = int(quantity_match.group(1))
    ...
    async def handle_message(self, user_message: str) -> str:
        ...
        """
        try:
            # בדיקה אם זו שאלת מידע
            if any(word in user_message.lower() for word in ["מה", "כמה", "איזה", "תראה", "הצג"]):
                from src.agents.information_agent import InformationAgent
                info_agent = InformationAgent()
                return await info_agent.handle_message(user_message)

            # בדיקה אם זו בקשה ליצירת מוצר
            if any(phrase in user_message.lower() for phrase in ["צור מוצר", "הוסף מוצר", "יצירת מוצר"]):
                product_data = self._extract_product_info(user_message)
                if product_data:
                    return self.create_product(product_data)
                else:
                    return (
                        "לא הצלחתי להבין את פרטי המוצר. אנא השתמש באחד מהפורמטים הבאים:\n"
                        "• צור מוצר חדש [שם המוצר] מחיר [מחיר] כמות [כמות]\n"
                        "• הוסף מוצר חדש בשם [שם המוצר] במחיר [מחיר]\n"
                        "• יצירת מוצר [שם המוצר] ב-[מחיר] שקל"
                    )
